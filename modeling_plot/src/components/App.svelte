<script>
    ...
    function play() {
      ...
      if (year < 1953) {
        ...
        }, Math.max(7000 - 1000 * (year - 1950), 0));
      } else {
        interval2 = setInterval(() => {
          if (year < 2022) { 
            // removeAnnotation();
    ...
    function appendAnnotation() {
      // Add your annotation at the specified coordinate
      const annotationX = 100; // Set your X-coordinate
      const annotationY = 100; // Set your Y-coordinate
      annotation = svg.append("g").attr("class", "annotation")
      annotation
        .append("text")
        .attr("x", annotationX)
        .attr("y", annotationY)
    ...
    function removeAnnotation() {
      // Remove the annotation from the SVG
